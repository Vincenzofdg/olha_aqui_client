...
function Ad({ navigation: { navigate }, route: { params } }) {
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Image source={{ uri: str.website + params.image }} style={styles.image} resizeMode="contain" />
            <View style={styles.contactContainer} onPress={() => links(params?.phone)}>
                {params?.whatsapp.length > 0 && (
                    <TouchableOpacity onPress={() => whatsapp(params?.whatsapp)}>
                        <Image source={WhatsImg} style={styles.phoneImg} resizeMode="cover" />
                    </TouchableOpacity>
                )}
                {params?.phone.length > 0 && (
                    <TouchableOpacity onPress={() => phone(params?.phone)}>
                        <Image source={PhoneImg} style={styles.phoneImg} resizeMode="cover" />
                    </TouchableOpacity>
                )}
            </View>
        </SafeAreaView>
